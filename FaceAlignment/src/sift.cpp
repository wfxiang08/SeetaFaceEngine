...
void SIFT::filter2(double *gray_im, double *kernel, int kernel_size, double *filter_im) {
    ...
    for (int i = pad_size; i < param.image_height + pad_size; i++) {
        ...
        for (int j = pad_size; j < param.image_width + pad_size; j++) {
            gray_img_ex[i * (param.image_width + (kernel_size - 1)) + j] = gray_im[(i - pad_size) * param.image_width +
                                                                                   (j - pad_size)];
        }
    }
...
void SIFT::CalcSIFT(BYTE *gray_im, double *sift_feature) {
    ...
    for (int location_x = param.patch_size / 2;
         location_x <= param.image_height - (param.patch_size / 2); location_x += param.grid_spacing) {
        for (int location_y = param.patch_size / 2;
             location_y <= param.image_width - (param.patch_size / 2); location_y += param.grid_spacing) {
            ...
            for (int p_x = -param.patch_size / 2;
                 p_x <= param.patch_size / 2 - param.sample_pixel; p_x += param.sample_pixel) {
                for (int p_y = -param.patch_size / 2;
                     p_y <= param.patch_size / 2 - param.sample_pixel; p_y += param.sample_pixel) {
