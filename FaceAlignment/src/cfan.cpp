...
/** Detect five facial landmarks, i.e., two eye centers, nose tip and two mouth corners.
  ...
  */
void CCFAN::FacialPointLocate(const unsigned char *gray_im, int im_width, int im_height, seeta::FaceInfo face_loc,
                              float *facial_loc) {
...
/** Extract shape indexed SIFT features.
  ...
  */
void CCFAN::TtSift(const unsigned char *gray_im, int im_width, int im_height, float *face_shape, int patch_size,
                   double *sift_fea) {
...
/** Extract a image patch which is centered at point(point_x, point_y) with a given patch size.
  ...
  */
void CCFAN::GetSubImg(const unsigned char *gray_im, int im_width, int im_height, float point_x, float point_y,
                      int patch_size, BYTE *sub_img) {
